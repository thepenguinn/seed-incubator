\subtikzpicturedef{subAbsSoilMoistureSensorSystem} {
    origin%
} {
    ...
    \ovrAbsSensorModuleDrawLabelName {#1-s1} {\texttt{SMS}}
    \ovrAbsSensorModuleDrawLabelName {#1-s0} {\texttt{SMS}}
    \ovrAbsSensorModuleDrawLabelName {#1-s2} {\texttt{SMS}}
    \ovrAbsSensorModuleDrawLabelName {#1-s3} {\texttt{SMS}}

    \ovrAbsMuxDrawLabelName {#1-mux} {\texttt{MUX}}
    ...
    (#1-mux-l0) ++(\absCtkPinPad, 0) node [right = 4pt] {\texttt{C3}}
    (#1-mux-l1) ++(\absCtkPinPad, 0) node [right = 4pt] {\texttt{C4}}
    (#1-mux-l2) ++(\absCtkPinPad, 0) node [right = 4pt] {\texttt{C5}}
    (#1-mux-l3) ++(\absCtkPinPad, 0) node [right = 4pt] {\texttt{C6}}
    ;
    ...
    %};
    %
    %\draw ($(#1-s1-center)!0.50!(#1-s2-center)$) coordinate (#1-tmp);
    %\node at (#1-tmp -| #1-leftVert) [
    %    absCtkAnnotation,
    %    anchor = east,
    %] {
    %    \absCtkAnnotationWrap{
    %    \item \textbf{SMS:} Soil Moisture Sensors.
    %    }
    %};
    %
...
\subtikzpictureactivate{subAbsSoilMoistureSensorSystem}
